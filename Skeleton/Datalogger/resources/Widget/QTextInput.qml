...
Item {
    ...
    TextInput {
        ...
        onCursorVisibleChanged: {
            if(!cursorVisible)
                Delivery.hmiEvent(this.objectName, doneEventID, text)
        }
    }
    ...
    MouseArea {
        ...
        onPressed: Delivery.hmiEvent(parent.objectName, CEMSEnum.HMI_TEXTINPUT_FOCUS_CHANGED, isFullKey ? "1" : "0")
    }

    function doneInput() {
        if(doneEventID != -1)
            Delivery.hmiEvent(this.objectName, doneEventID, txtInp.text)
    }
}
