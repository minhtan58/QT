...
{
    m_topBarView->setProperty("stationId",      QVariant::fromValue(GETDPDATA(HMIEvents::DP_GENERAL_STATION_ID)));
    m_topBarView->setProperty("stationName",    QVariant::fromValue(GETDPDATA(HMIEvents::DP_GENERAL_STATION_NAME)));
    m_topBarView->setProperty("systemDate",     QVariant::fromValue(GETDPDATA(HMIEvents::DP_SYS_DATE)));
    m_topBarView->setProperty("systemTime",     QVariant::fromValue(GETDPDATA(HMIEvents::DP_SYS_TIME)));
}
...
void TopBarAdapter::updateAppData(int dpid)
{
    ...
    case HMIEvents::DP_GENERAL_STATION_NAME: {
        m_topBarView->setProperty("stationName", QVariant::fromValue(data));
        break;
    }
    case HMIEvents::DP_STATUS_ALARM_ON_COUNT: {
