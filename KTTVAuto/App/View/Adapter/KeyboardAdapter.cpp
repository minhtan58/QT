...
{
    DLOG("create keyboard, mode - %d, objectName - %s", mode, m_currentTextInput->objectName().toStdString().data());
    ...
    //exception
    if(mode == KB_MODE_FULLKEY)  {
        if(m_currentTextInput->objectName() == "login_user" || m_currentTextInput->objectName() == "login_passwd") {
    ...
    case CEMS_KEYBOARD_KEY_TYPE_CTRL: {
        if(keyValue == "hide")
            ...
            if(m_currentTextInput) {
                ...
                m_currentTextInput->setProperty("cursorVisible", false);
                hideKeyboard();
            }
        }
        else {}
