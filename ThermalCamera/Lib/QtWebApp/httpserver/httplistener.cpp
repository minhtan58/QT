...
void HttpListener::incomingConnection(tSocketDescriptor socketDescriptor) {
    ...
    if (freeHandler)
    {
        ...
        connect(this,SIGNAL(handleConnection(tSocketDescriptor)),freeHandler,SLOT(handleConnection(tSocketDescriptor)), Qt::QueuedConnection);
    ...
    else
    {
        ...
        connect(socket, SIGNAL(disconnected()), socket, SLOT(deleteLater()), Qt::QueuedConnection);
